...
% Number of frames
n = size(Segment(3).Q,3); % At least Q parameter for foot (or hand) segment

% Frames of interest
ni = [1:1:n]; % 1% 50% and 100% of cycle
...
% % Centre of mass
% G1 = Mprod_array3(Q2Tu_array3(Segment(2).Q),...
...
% for e = 1:size(Muscle,1)
%     for f = 1:length(Muscle(e).muscle_point)-1
%             MP1(:,:,ni) = Mprod_array3(Muscle(e).muscle_point(f).N(:,:,ni),Segment(translator(Muscle(e).muscle_point(f).nbody)).Q(:,:,ni));
%             %MP2(:,:,ni) = Mprod_array3(Muscle(e).muscle_point(f+1).N(:,:,ni),Segment(translator(Muscle(e).muscle_point(f+1).nbody)).Q(:,:,ni));
%             MP1x=(permute(MP1(1,1,ni),[3,2,1]));
%             MP1y=(permute(MP1(2,1,ni),[3,2,1]));
%             MP1z=(permute(MP1(3,1,ni),[3,2,1]));
% %             MP2x=(permute(MP2(1,1,ni),[3,2,1]));
% %             MP2y=(permute(MP2(2,1,ni),[3,2,1]));
% %             MP2z=(permute(MP2(3,1,ni),[3,2,1]));
%             plot3(MP1x,MP1y,MP1z,'.r')
% %             plot3(MP2x,MP2y,MP2z,'.r');
% %             plot3([MP1x,MP2x]',[MP1y,MP2y]',[MP1z,MP2z]','Color','r','Linewidth',2);
%     end
% end
...
% % Centre of mass
% G2 = Mprod_array3(Q2Tu_array3(Segment(3).Q),...
...
% % Centre of mass
% G3 = Mprod_array3(Q2Tu_array3(Segment(4).Q),...
...
% % Centre of mass
% G4 = Mprod_array3(Q2Tu_array3(Segment(5).Q),...
...
% % Torso
% % Proximal enpoint P
% P5x=(permute(Segment(6).Q(4,1,ni), [3,2,1]));
% P5y=(permute(Segment(6).Q(5,1,ni), [3,2,1]));
% P5z=(permute(Segment(6).Q(6,1,ni), [3,2,1]));
% plot3(P5x,P5y,P5z,'om');
% % Distal endpoints D
% D5x=(permute(Segment(6).Q(7,1,ni), [3,2,1]));
% D5y=(permute(Segment(6).Q(8,1,ni), [3,2,1]));
% D5z=(permute(Segment(6).Q(9,1,ni), [3,2,1]));
% plot3(D5x,D5y,D5z,'.m');
% plot3([P5x,D5x]',[P5y,D5y]',[P5z,D5z]','m');
% % U axis
% U5x=(permute(Segment(6).Q(1,1,ni), [3,2,1]));
% U5y=(permute(Segment(6).Q(2,1,ni), [3,2,1]));
% U5z=(permute(Segment(6).Q(3,1,ni), [3,2,1]));
% quiver3(P5x,P5y,P5z,U5x,U5y,U5z,0.5,'m');
% % W axis
% W5x=(permute(Segment(6).Q(10,1,ni), [3,2,1]));
% W5y=(permute(Segment(6).Q(11,1,ni), [3,2,1]));
% W5z=(permute(Segment(6).Q(12,1,ni), [3,2,1]));
% quiver3(D5x,D5y,D5z,W5x,W5y,W5z,0.5,'m');
% % Markers
