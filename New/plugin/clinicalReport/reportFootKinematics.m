...
function f = reportFootKinematics(Patient,Session,Condition,pluginFolder,normFolder,norm)
...
Norm.KinematicsFoot = temp.Normatives.RkinematicsFoot;
% Norm.Event = temp.Normatives.Rphases;
cd(pluginFolder);
for i = 1:size(Condition,2)
    RkinematicsFoot(i) = Condition(i).All.RkinematicsFoot;
%     LkinematicsF(i) = Condition(i).All.LkinematicsFoot;
...
if size(Condition,2) == 1
    ...
    for i = 1:size(Condition,2)
        ...
        for j = 1:length(Condition(i).Gait);
            if ~isempty(Condition(i).Gait(j).RkinematicsFoot.FootSag)
                nbtrials = nbtrials+1;
            end
        end
        % Write the legend
    ...
    % Get recorded Foot kinematics right side
    % ---------------------------------------------------------------------
    names = fieldnames(Condition.All.RkinematicsFoot);
    ...
    for igraph=1:9
        ...
        Rkinematics = eval(['RkinematicsFoot.',names{igraph},'']);
...
%         Lkinematics = eval(['LkinematicsFoot.',names{igraph},'']);
...
%         plot([Levent(i).p4.mean Levent(i).p4.mean],[YL(1)-20 YL(1)+(YL(2)-YL(1))/10],'Linewidth',1.5,'Color',colorL(i,:)); %CHS
    ...
    for igraph=1:9
        ...
        corridor(inorm.mean,inorm.std,[0.5 0.5 0.5]);
        for i = 1:size(Condition,2)
            Rkinematics(i) = eval(['RkinematicsFoot(i).',names{igraph},'']);
...
%         plot([Revent(i).p4.mean Revent(i).p4.mean],[YL(1)-20 YL(1)+(YL(2)-YL(1))/10],'Linewidth',1.5,'Color',colorR(i,:)); %CHS
    ...
    % Get recorded Foot kinematics right side
    % ---------------------------------------------------------------------
    names = fieldnames(Condition.All.LkinematicsFoot);
    ...
    for igraph=1:9
        ...
        corridor(inorm.mean,inorm.std,[0.5 0.5 0.5]);
        for i = 1:size(Condition,2)
            Lkinematics(i) = eval(['LkinematicsFoot(i).',names{igraph},'']);
