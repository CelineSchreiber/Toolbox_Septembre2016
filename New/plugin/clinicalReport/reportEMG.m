...
if ~isempty(Condition(icondition).Gait(itrial).Lemg)
    ...
    for j = 1:length(names)
        ...
        if strfind(cell2mat(names(j)),'_filt')
            Lemg_filt.mean(1:size(eval(['Condition(icondition).All.Lemg.',cell2mat(names(j)),'.mean;']),1),k2) = ...
                eval(['Condition(icondition).All.Lemg.',cell2mat(names(j)),'.mean;']);
            Lemg_filt.std(1:size(eval(['Condition(icondition).All.Lemg.',cell2mat(names(j)),'.mean;']),1),k2) = ...
                eval(['Condition(icondition).All.Lemg.',cell2mat(names(j)),'.std;']);
            k2 = k2+1;
        end
    end
