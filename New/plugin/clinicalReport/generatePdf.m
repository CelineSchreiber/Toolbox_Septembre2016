...
function generatePdf(f,pluginFolder,filesFolder,filename)
...
if strfind(filename,'firstPage')
    if f(1) ~= 0
        print(f(1),'-dpdf','-r300',[filename,'_',num2str(length(list)+1),'_temp.pdf']);
    end
    if length(f) > 1
        print(f(2),'-dpdf','-r300',[filename,'_',num2str(length(list)+2),'_temp.pdf']);
    end
end
