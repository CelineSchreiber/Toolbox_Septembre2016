...
% Initialisation
% =========================================================================
clearvars -except database matFolder toolboxFolder reportFolder;
clc;
warning('off','All');
cd(toolboxFolder);
...
c(1) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportFirstPage(Patient,Pathology,Treatment,Session,Condition,pluginFolder);filename = ''firstPage''; generatePdf(d,f,pluginFolder,matFolder,filename);');
c(2) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .390 .25 .04],'String','Gait Parameters','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportGaitParameters(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''gaitParameters''; generatePdf(d,f,pluginFolder,matFolder,filename);');
c(3) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .340 .25 .04],'String','Kinematics','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportKinematics(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinematics''; generatePdf(d,f,pluginFolder,matFolder,filename);');
...
c(4) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportKinetics(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinetics''; generatePdf(d,f,pluginFolder,matFolder,filename);');
...
c(5) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportEMG(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''EMG''; generatePdf(d,f,pluginFolder,matFolder,filename);');
c(6) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .190 .25 .04],'String','Posturo','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportRachis(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''rachis_kinematics''; generatePdf(d,f,pluginFolder,matFolder,filename);f = reportAI(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''AI''; generatePdf(d,f,pluginFolder,matFolder,filename);f=reportNCCF(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''NCCF''; generatePdf(f,pluginFolder,matFolder,filename);');
...
c(7) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportIndexes(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''indexes''; generatePdf(d,f,pluginFolder,matFolder,filename);');
c(8) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .090 .25 .04],'String','Upper Limbs','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportUpperLimbsKinematics(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinematicsUL''; generatePdf(d,f,pluginFolder,matFolder,filename);');
e(4) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.350 .090 .25 .04],'String','> Check 1 trial','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportUpperLimbsTrial(Patient,Session,Condition,pluginFolder,normFolder,norm);');
c(9) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    [.050 .040 .25 .04],'String','Foot Kinematics (Baropodo)','call',...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); f = reportKinematicsFootOnly(Patient,Session,Condition,pluginFolder,normFolder,norm);filename = ''kinematics''; generatePdf(d,f,pluginFolder,matFolder,filename);');
...
d(1) = uicontrol('Style','pushbutton','Units','normalized','Position',...
    ...
    '[Patient,Pathology,Treatment,Session,Condition] = mergeConditions(temp,a,b); mergePdf(Patient,Session,reportFolder,toolboxFolder,pluginFolder,matFolder,database)');
...
d(4) = uicontrol('Style','checkbox','Units','normalized','Value',0,'String','write in pdf','Position',...
        [.350 .440 .25 .03],'Callback','');